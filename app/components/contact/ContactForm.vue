...
const status = ref<{ state: '' | 'ok' | 'error'; message: string }>({
  state: '',
  message: ''
})
...
function validate(): boolean {
  ...
  errors.phone =
    RE_MOBILE.test(p) || RE_LANDLINE.test(p) ? '' : '请填写 11 位手机号，或带区号的座机号'
...
async function onSubmit() {
  ...
  try {
    ...
    status.value = {
      state: 'ok',
      message: '已收到，我们会在一个工作日内按你留的电话回电。'
    }
  } catch (err) {
...
<template>
  <form class="space-y-5" novalidate @submit.prevent="onSubmit">
    <div>
      <label for="cf-company" class="block text-sm font-medium">公司名称</label>
      <input
        id="cf-company"
        v-model="company"
        type="text"
      ...
      />
      <p v-if="errors.company" class="mt-2 text-xs text-error" role="alert">
        {{ errors.company }}
      </p>
    </div>

    <div>
      <label for="cf-name" class="block text-sm font-medium">联系人姓名</label>
      <input
        id="cf-name"
        v-model="name"
        type="text"
      ...
      />
      <p v-if="errors.name" class="mt-2 text-xs text-error" role="alert">
        {{ errors.name }}
      </p>
    </div>

    <div>
      <label for="cf-phone" class="block text-sm font-medium">联系电话</label>
      <input
        id="cf-phone"
        v-model="phone"
        type="tel"
      ...
      />
      <p v-if="errors.phone" class="mt-2 text-xs text-error" role="alert">
        {{ errors.phone }}
      </p>
      <p v-else class="mt-2 text-xs text-muted">回电只用这个号码，请填写商务时间方便接听的</p>
    </div>

    <!-- honeypot：对真人不可见（CSS 移出视口 + tabindex -1），爬虫填了即被识别 -->
    <div class="absolute -left-[9999px] -top-[9999px] size-px overflow-hidden" aria-hidden="false">
      <label for="cf-website">网站</label>
      <input id="cf-website" v-model="website" type="text" tabindex="-1" autocomplete="off" />
    </div>
    ...
    <p
      ...
      class="text-sm"
      :class="
        status.state === 'ok' ? 'text-success' : status.state === 'error' ? 'text-error' : undefined
      "
    >
      {{ status.message }}
    </p>
